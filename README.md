...
3. **LF/HF 비율 (Low Frequency/High Frequency Ratio)**: 주파수 도메인에서의 HRV 파라미터로, 저주파와 고주파 영역 사이의 비율을 나타냅니다. 높은 LF/HF 비율은 스트레스 또는 심혈관 질환과 관련될 수 있다.


   HRV (Heart Rate Variability) 분석에서 사용되는 주요 지표들인 RMSSD, SDNN, 그리고 LF/HF 비율의 계산 방법을 설명하겠습니다. 이러한 지표들은 심장의 시간적 변동성을 측정하여 신체의 스트레스 수준, 심장 건강 상태, 그리고 자율신경계의 활동을 평가하는 데 사용됩니다.

RMSSD (Root Mean Square of Successive Differences)
RMSSD는 연속하는 RR 간격(심장 박동 사이의 시간) 차이의 제곱근 평균을 나타냅니다. 이 지표는 주로 심장의 빠른 변화를 반영하며, 특히 자율신경계 중 부교감신경계의 활성을 나타냅니다.
...
SDNN은 모든 RR 간격의 표준편차를 나타냅니다. 이 지표는 심장 박동의 전체적인 변동성을 나타내며, 자율신경계의 전반적인 활동과 관련이 있습니다.
...
LF/HF Ratio
LF (Low Frequency) 및 HF (High Frequency) 비율은 주파수 영역 분석에서 계산됩니다. LF는 0.04에서 0.15Hz 사이, HF는 0.15에서 0.4Hz 사이의 주파수를 포함합니다. LF는 자율신경계의 교감 및 부교감 신경 활동을 모두 반영하며, HF는 주로 부교감신경 활동을 반영합니다. LF/HF 비율은 교감/부교감신경계의 균형을 나타내는 지표로 사용됩니다.

공식:
LF 및 HF의 파워는 주로 주파수 영역 분석을 통해 계산되며, LF/HF 비율은 다음과 같이 계산됩니다.
LF/HF Ratio
=
LF Power
HF Power
